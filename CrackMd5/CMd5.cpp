...
std::string CMd5::Md5(const std::string &src)
{
	char res[17];
	MD5Init(&m_md5Ctx);
	...
	return ToHex(std::string(res, 16));
}

bool CMd5::IsMd5Data(const std::string &md5)
{
    return (32 == md5.size());
}
...
std::string CMd5::ToHex(const std::string &str)
{
	int j = 0;
	int size = str.size();
...
std::string CMd5::FromHex(const std::string &hex)
{
	std::string res((hex.size() + 1)/ 2, 0);
